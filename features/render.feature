...
機能: テンプレートからのプロジェクト生成
  ...
  シナリオ: 生成先のシンボリックリンクをたどらない
    前提 生成先のファイル "README.md" が外部ファイルへのシンボリックリンクである
    もし シンボリックリンクがある生成先へ "relens new python-lib" を実行する
    ならば 終了コードは 0 ではない
    かつ エラーはシンボリックリンクへの書き込み拒否を示す
    かつ 外部ファイルの内容は変更されていない
